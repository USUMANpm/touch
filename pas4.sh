...
# - сеть: 192.168.0.0/24
# - интерфейс: enp7s2
# - пул: 192.168.0.10-192.168.168.200
# - шлюз: 192.168.0.1
# - DNS: 10.0.0.1
...
# Данные для отправки (студент должен ввести свое имя или оно берется из системы)
STUDENT_NAME="${1:-$(whoami)}" # Имя можно передать как аргумент или взять логин
...
JSON_DATA=$(cat <<EOF
{
  ...
  "details": {
    ...
    "checks": [
      $(grep -q "✓" <<< "$(systemctl is-active dhcpd)" && echo '{"item": "service", "status": "ok"},')
      # ... здесь можно сформировать более детальный отчет по каждому пункту
    ]
  }
}
EOF
...
# Отправляем POST-запрос (используем curl)
curl -s -X POST \
  -H "Content-Type: application/json" \
  -d "$JSON_DATA" \
  https://script.google.com/macros/s/AKfycbxxA779spRj6nUHz0byxxP7Ku0zxj2FWDK43a7-ElX5AbguBKPWeF28kWJQhTGLNkxm/exec

# --- КОНЕЦ НОВОГО БЛОКА ---

# Далее идет сохранение отчета в файл и вывод
echo -e "\n${GREEN}Отчет сохранен в: ${REPORT_FILE}${NC}"
